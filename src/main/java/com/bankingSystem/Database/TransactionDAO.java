...
public class TransactionDAO {
    ...
    private Transaction buildTransactionFromResultSet(ResultSet rs) throws SQLException {
        ...
        if (from == null && to == null) {
            tx = new Transaction(type, amount);
        } else {
            tx = new Transaction(type, amount, from, to);
        }

        // تعيين البيانات الأخرى
        tx.setStatus(rs.getString("status"));
        // نحتاج إلى حقن transactionId و timestamp يدويًا لأن المُنشئ لا يقبلها
        // لذلك نستخدم reflection أو نضيف constructor خاص (الحل الأنظف هو إضافة constructor في Transaction)

        // حل مؤقت وأنيق: نضيف constructor في Transaction يقبل كل الحقول
        // لكن حاليًا نستخدم هذا الحل البسيط:
        java.lang.reflect.Field idField = null;
        java.lang.reflect.Field tsField = null;
        try {
            idField = Transaction.class.getDeclaredField("transactionId");
            tsField = Transaction.class.getDeclaredField("timestamp");
        ...
        try {
            idField.set(tx, rs.getString("transactionId"));
            tsField.set(tx, rs.getObject("timestamp", LocalDateTime.class));
        } catch (Exception e) {
            e.printStackTrace();
        }
